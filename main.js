...
function genTableRefDef_v3(oasModel, resource, dataSet, tableRef) {
    ...
    Object.keys(definition.items.properties).forEach(function (propertyName) {
        if (propertyName.charAt(0) !== "_") {
            ...
            oasModel.components.schemas[tableRefItem].properties[propertyName].type = definition.items.properties[propertyName].type;
            if (definition.items.properties[propertyName].type === "array") {
                oasModel.components.schemas[tableRefItem].properties[propertyName].items = definition.items.properties[propertyName].items;
            }
        }
    });
}
